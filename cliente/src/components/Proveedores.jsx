...
const Proveedores = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const requiredFields = ['nombre', 'direccion', 'telefono', 'ruc'];
    ...
    try {
      if (editingId) {
        await axios.patch(`http://localhost:8000/proveedores/${editingId}`, {
          nombre,
          direccion,
          telefono,
          ruc
        ...
        const updatedProveedores = proveedores.map((proveedor) => {
          if (proveedor._id === editingId) {
            return { ...proveedor, nombre, direccion, telefono, ruc };
          }
          return proveedor;
        });
        ...
        const response = await axios.post('http://localhost:8000/proveedores', {
          nombre,
          direccion,
          telefono,
          ruc
  ...
  const handleEdit = async (id) => {
    try {
      const response = await axios.get(`http://localhost:8000/proveedores/${id}`);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:8000/proveedores/${id}`);
  ...
  return (
    <div>
      <form onSubmit={handleSubmit} className="form-container cargarproducto">
        <h1 className='bi bi-file-earmark-plus-fill titulodeclientes'><strong>CARGAR PROVEEDORES</strong></h1>
        <div className='cajadeclientes'>
          <div>
            <label>
              Nombre:
              <input className={`form-control ms-2 ${missingFields.includes('nombre') ? 'is-invalid' : ''}`} type="text" value={nombre} onChange={(e) => setNombre(e.target.value)} />
            </label>
            <br />
            <label>
              Dirección:
        ...
        <button className='btn btn-success bi bi-plus-circle clientes' type="submit">{editingId ? 'Actualizar Proveedor' : 'Crear Proveedor'}</button>
      </form>
      <br />
      <button className='btn btn-success bi bi-plus-circle' onClick={handleToggleModal} style={{ marginLeft: '5%' }}>Ver Proveedores cargados</button>
      {modalVisible && (
