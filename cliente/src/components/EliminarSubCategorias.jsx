...
const EliminarSubcategorias = () => {
  ...
  useEffect(() => {
    const obtenerCategoria = async () => {
      try {
        const response = await axios.get(`https://node-mongo-api4.onrender.com/categories/${id}`);
  ...
  const eliminarSubcategoria = async () => {
    try {
      await axios.put(`https://node-mongo-api4.onrender.com/categories/${id}/subcategories`, {
